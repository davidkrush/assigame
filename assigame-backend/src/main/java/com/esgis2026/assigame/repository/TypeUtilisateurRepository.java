...
public interface TypeUtilisateurRepository extends JpaRepository<TypeUtilisateur, Long> {
    ...
    @Query("SELECT t FROM TypeUtilisateur t WHERE LOWER(t.nom_typeutilisateur) LIKE LOWER(CONCAT('%', :motCle, '%'))")
    Optional<TypeUtilisateur> findFirstByNomContaining(@Param("motCle") String motCle);
}
